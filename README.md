...
Para ejecutar la integración Supabase, define `RUN_SUPABASE_INTEGRATION=true` y
`DEMO_USER_PASSWORD`. El test negativo de `member` usa por defecto
`demo-member@test.com` en la org seed `11111111-1111-1111-1111-111111111111`,
con la misma contraseña demo (se puede sobrescribir con `MEMBER_USER_*`).
...
| `DEMO_USER_PASSWORD` | Login demo-a / demo-b / demo-member en tests RLS y E2E |
| `MEMBER_USER_EMAIL` / `MEMBER_USER_PASSWORD` / `MEMBER_ORGANIZATION_ID` | Opcionales: sobrescriben la identidad member de seed (`demo-member@test.com`) |

En pull requests externas la integración Supabase no puede usar secretos; en
`main`/`master` el gate RLS requiere `DEMO_USER_PASSWORD` (y la URL/anon key).
La identidad `member` es la de seed (`demo-member@test.com`) salvo override.
